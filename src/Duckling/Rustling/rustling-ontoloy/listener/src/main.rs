...
fn main() {
    let matches = clap_app!(rustling_cli =>
        ...
        (@subcommand listen =>
             (@arg input: -i --input +required +takes_value "Path to input file")
             (@arg output: -o --output +required +takes_value "Path to output file")
        )
        (@subcommand play =>
             (@arg kinds: -k --kinds +takes_value +use_delimiter "kinds, last one wins, coma separated")
             (@arg sentence: +required "Sentence to test")
    ...
    match matches.subcommand() {
        ...
        ("listen", Some(matches)) => {
            ...
            // never ending:
            let polltime = time::Duration::from_millis(100);
            let mut done = false;
            while !done {
                while !fs::metadata(input).is_ok() {
                    thread::sleep(polltime);
                }

                let in_string = fs::read_to_string(input).expect("Unable to read file");
                let in_json: serde_json::Value = serde_json::from_str(&in_string).unwrap();
                let sentence = &in_json["sentence"].as_str().unwrap();
                ...
                for (ix, c) in entities.iter().enumerate().rev() {
                    println!("{}", format!("{:?}", c.value));
                    fs::write(output, format!("{:?}", c.value)).expect("Unable to write file");
                }
